...
The `lib/` directory contains default model templates which are structured as
directoris containing a `*.sbatch` job submission script. The model templates
can also use scripts in `utils/` (which is design to keep scripts usable by
multiple model templates and can be set by the `UTILS` variable in the config
file).
...
the selected model template, `parallel` and `R` in your `PATH` variable (for
example, using `module load`). The `qqman` and `data.table` R packages should
also be installed.

To run the pipeline, specify the config file and a model template directory.
...
config file. Check the exsiting model templates for examples. 
